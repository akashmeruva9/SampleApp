...
fun CircularAmountPicker(
    ...
    Canvas(modifier = modifier
        ...
        drawLine(
            color = LightGreen,
            strokeWidth = ((stroke*3.0)/3.0).toFloat(),
            start = center + Offset(
                (radius-5) * kotlin.math.cos((-90 + abs(appliedAngle)) * PI / 180f).toFloat(),
                (radius-5) * kotlin.math.sin((-90 + abs(appliedAngle)) * PI / 180f).toFloat()
            ),
            end = center + Offset(
                (radius+5) * kotlin.math.cos((-90 + abs(appliedAngle)) * PI / 180f).toFloat(),
                (radius+5) * kotlin.math.sin((-90 + abs(appliedAngle)) * PI / 180f).toFloat()
